...
from RiruruMusic.utils import AdminRightsCheck
from RiruruMusic.utils.inline.speed import speed_markup
from RiruruMusic.utils.inline.play import close_keyboard
from RiruruMusic.utils.decorators.language import languageCB
from RiruruMusic.utils.database.memorydatabase import is_active_chat, is_nonadmin_chat
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if checkspeed:
        if str(checkspeed) == str(speed):
            if str(speed) == "1.0":
                speed = "ɴᴏʀᴍᴀʟ"
            return await CallbackQuery.answer(f"» ᴀʟʀᴇᴀᴅʏ ᴘʟᴀʏɪɴɢ ᴏɴ {speed} sᴘᴇᴇᴅ.", show_alert=True)
    else:
