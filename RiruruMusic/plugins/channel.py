...
from RiruruMusic.utils.database import set_cmode
from RiruruMusic.utils.decorators.admins import AdminActual
...
async def playmode_(client, message: Message, _):
    ...
    if (str(query)).lower() == "disable":
        ...
        if creatorid != message.from_user.id:
            return await message.reply_text(_["cplay_6"].format(chat.title, creatorusername))
