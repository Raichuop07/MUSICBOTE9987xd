...
from RiruruMusic import Telegram, YouTube, app
from RiruruMusic.misc import SUDOERS
from strings import get_command, get_string
from RiruruMusic.plugins.playlist import del_plist_msg
from RiruruMusic.plugins.sudoers import sudoers_list
from RiruruMusic.utils.database import (add_served_chat,
                                       add_served_user,
                                       get_served_chats,
                                       get_served_users,
                                       blacklisted_chats,
            ...
            key = InlineKeyboardMarkup(
                [
                    [
                        ...
                        InlineKeyboardButton(text="• sᴜᴩᴩᴏʀᴛ •", url="https://t.me/AltronChats")
                    ],
                ]
            )
            await m.delete()
    ...
    for member in message.new_chat_members:
        try:
            ...
            if member.id == app.id:
                ...
                await message.reply_photo(
                    photo=config.START_IMG_URL,
                    caption=_["start_3"].format(config.MUSIC_BOT_NAME, userbot.username, userbot.id),
                    reply_markup=InlineKeyboardMarkup(out),
                )
